...
func quantityRegistersTagsSlave(tag *models.Tag) uint16 {
	if tag == nil {
		return 1
	}
	switch tag.TypeID {
...
func quantityBitsTagsSlave(tag *models.Tag) uint16 {
	return 1
}

func createReadSlaveInBlocks(tags []*models.Tag, maxQuantity int, quantityFunc func(*models.Tag) uint16) []readBlock {
	if maxQuantity <= 0 {
		maxQuantity = 1
	}
...
func groupTagsByOperationSlave(tags []*models.Tag) map[uint][]*models.Tag {
	tagsByOperation := make(map[uint][]*models.Tag)
	for _, tag := range tags {
		if tag == nil {
			continue
		}
		operationID := operationIDSlave(tag)
		tagsByOperation[operationID] = append(tagsByOperation[operationID], tag)
	}
	return tagsByOperation
}

func operationIDSlave(tag *models.Tag) uint {
	if tag == nil {
